// O4AA Config Clipboard — Enhanced Bookmarklet
// Injects a floating panel, toast notifications, and contextual selection assignment.
...
function merge(prev,vals){var m={};ALL_KEYS.forEach(function(k){m[k]=vals[k]||prev[k]||''});return m}
...
function extract(){
  ...
  if(p.match(/\/admin\/app\/oidc_client\/instance\//)){
    ...
    if(!vals.clientId){var m=document.body.innerText.match(/\b(0oa[a-zA-Z0-9]{10,})\b/);if(m)vals.clientId=m[1]}
  }
  if(p.match(/\/admin\/people\/workload\//)){
    var wm=p.match(/\/workload\/(wlp_[a-zA-Z0-9]+)/);
    if(wm)vals.agentClientId=wm[1];
    if(!vals.agentClientId){var wm2=document.body.innerText.match(/\b(wlp_[a-zA-Z0-9]+)\b/);if(wm2)vals.agentClientId=wm2[1]}
  }
...
function showSelectionDropdown(sel,state,onPick){
  ...
  preview.style.cssText='padding:6px 12px;font-size:11px;color:#999;font-family:monospace;border-bottom:1px solid #eee;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;';
...
function renderPanel(state){
  ...
  copyBtn.onclick=function(){
    var cfg={};ALL_KEYS.forEach(function(k){cfg[k]=state[k]||''});
    navigator.clipboard.writeText(JSON.stringify(cfg,null,2)).then(function(){
      showToast('Copied '+filled+'/'+total+' fields to clipboard!','success');
  ...
  clearBtn.onclick=function(){sessionStorage.removeItem(S);renderPanel({});showToast('Config cleared','warn')};
...
if(sel&&sel.length>0&&sel.length<5000){
  ...
  else showToast('Config panel open — '+countFilled(state)+'/'+ALL_KEYS.length+' fields','warn');
}
